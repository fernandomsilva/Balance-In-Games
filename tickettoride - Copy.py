...
class Game:
	...
	def get_possible_moves(self, player_index):
		...
		if self.players_choosing_destination_cards == True:
			dest_card_set = list_pending_destination_cards(player_index)
			for x in range(2, len(dest_card_set) + 1):
				comb = itertools.combinations(dest_card_set, x)
				for cardset in comb:
					pmoves.append(Move(move_choose_destination_cards, [player_index, cardset]))
		else if self.players[player_index].choosing_destination_cards == True:
			dest_card_set = list_pending_destination_cards(player_index)
			for x in range(1, len(dest_card_set) + 1):
				comb = itertools.combinations(dest_card_set, x)
				for cardset in comb:
					pmoves.append(Move(move_choose_destination_cards, [player_index, cardset]))
		else if self.players[player_index].drawing_train_cards == True:
			pmoves.append(Move(move_drawTrainCard, ['top']))
			for card in self.train_cards_face_up:
				pmoves.append(Move(move_drawTrainCard, [card]))
		else:
			...
			for city1 in self.board.graph:
				for city2 in self.board.graph:
					for color in colors:
						edge = self.board.get_free_connection(city1, city2, color, self.number_of_players)
						if edge != None:
							if checkPlayerHandRequirements(self, player_index, edge['weight'], color) == True:
								pmoves.append(Move(move_claimRoute, [city1, city2, color]))
			pmoves.append(Move(move_drawDestinationCards,[]))
			pmoves.append(Move(move_drawTrainCard, ['top']))
			for card in self.train_cards_face_up:
				pmoves.append(Move(move_drawTrainCard, [card]))
