...
class OneStepThinkerAgent:
	...
	def decide(self, game, pnum):
		...
		if self.current_objective_color != 'GRAY':
			for move in draw_train_card_moves:
				if move.args == self.current_objective_color:
					return move
				if move.args == 'WILD':
					draw_wild_move = move
				if move.args == 'TOP':
	...
	def generate_game_plan(self, game, pnum):
		...
		if city1 == None:
			...
			if min_number_of_trains >= min_trains_threshold:
				#for move in possible_moves:
				#	if move.function == 'drawDestinationCards':
				#		return move
				return ['drawDestination', 'drawDestination', 'drawDestination']
			else:
				result = self.chooseMaxRoute(self, game, pnum);
	...
	def destinations_not_completed(self, game, pnum, joint_graph):
		...
		for card in destination_cards:
			...
			if not solved:
				if nx.has_path(joint_graph, city1, city2):
					try:
	...
	def chooseMaxRoute(self, game, pnum):
		...
		for city1 in free_routes_graph:
			for city2 in free_routes_graph[city1]:
				for edge in free_routes_graph[city1][city2]:
